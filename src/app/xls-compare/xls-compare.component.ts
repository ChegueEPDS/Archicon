...
export class XlsCompareComponent {
  ...
  private processApiResponse(response: { changes: string[]; fileUrl: string }): void {
    ...
    this.sanitizedComparisonResult.set([
      this.formatGroup("🔄 Módosított sorok:", groupedChanges.modified),
      this.formatGroup("✅ Új sorok", groupedChanges.new),
      this.formatGroup("❌ Törölt sorok", groupedChanges.deleted)
    ]);
